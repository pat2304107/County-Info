...
function Sidebar({ isMenuOpen, onLinkClick }) {
  return (
    <aside
      className={`${isMenuOpen ? 'block' : 'hidden'} z-20 bg-primary-light w-60 h-screen p-6 md:flex flex-col border-r border-secondary fixed left-0 top-0`}
    >
      <div className="mb-8">
        <Link to="/" className="text-2xl font-bold text-white mb-2" onClick={onLinkClick}>
          南投縣
        </Link>
      ...
      <nav className="flex-1 space-y-4">
        <Link
          to="/login"
          className="flex items-center text-white hover:text-secondary transition md:hidden"
          onClick={onLinkClick}
        >
          <span className="material-icons mr-2">login</span>登入
        </Link>
        <Link
          to="/map"
